...
- (NSPersistentStoreCoordinator *) persistentStoreCoordinator {
   ...
   if (![persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:localStoreURL options:options error:&error]) {
      NSLog(@"PSC:Unresolved error %@, %@", error, [error userInfo]);
      abort();
   }
   ...
   int j = 0;
	...
	while (aCategory = [enumerator nextObject]) {//sortIndex NSNumber
      ...
      if(result == NSOrderedSame){
         ...
         [aCategory setSortIndex:[NSNumber numberWithInt:j++]];
   }
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification{
   ...
   NSFileManager *fm = [NSFileManager defaultManager];
   /*
   if(![fm moveItemAtURL:localStoreURL toURL:tempLocalStoreURL error:error]){
      DLog(@"Error moving appSupportDb:%@ \nto tempFile in same Dir",[error userInfo]);
   }
   
   BOOL success = [self migrateUsingPSC:localStoreURL];
    */
   /*
   [defaults setObject:nil  forKey:@"DG_HM_DataVersionNumber"];
   [defaults setObject:nil  forKey:@"DG_HM_AppSupportDbPresentAtLoad"];
    */
   //DLog(@"self.localStoreURL.path=%@",self.localStoreURL.path);
   // /Users/mbarron/Library/Containers/com.DrummingGrouse.HungryMe/Data/Library/Application Support/HungryMe/HM_Mac_Recipes.sqlite
   NSError *error = nil;
#ifdef VERSION_1
   // manually move DB into AppSupport Dir
   [defaults setObject:@"1.0" forKey:DG_DEFAULTS_HM_APP_VERSION_KEY ];//@"DG_HM_App_Version";
   [defaults setObject:@"1.45" forKey:DG_DEFAULTS_HM_DATA_VERSION_KEY];
   abort();
#endif
#ifdef VIRGIN
   ...
   [defaults setObject:nil forKey:DG_DEFAULTS_HM_DATA_VERSION_KEY];//@"DG_HM_DataVersion";
   [defaults synchronize];
 
   if(![fm removeItemAtPath:self.localStoreURL.path  error:&error ]){
      DLog(@"Error removing appSupportDb:%@",[error userInfo]);
   }
   if(![fm removeItemAtPath:self.tempLocalStoreURL.path  error:&error ]){
   ...
   error=nil;
   if(![self.fileManager removeItemAtPath:[shmURL path] error:&error ]){
      DLog(@"Error removing shm file:%@",[error userInfo]);
   }
   error=nil;
   if(![self.fileManager removeItemAtPath:[walURL path] error:&error ]){
      DLog(@"Error removing wal file:%@",[error userInfo]);
   } */
   abort();
#endif
   ...
   /////  FONT  END  WORKING
   //DLog(@"self.localStoreURL=\n%@",self.localStoreURL);
   NSArray *versionArray=nil;
   NSError* error3 = nil;
   
   //NSNumber* verNumXml = nil;
   NSString* verStringApp = nil;
   ...
   NSComparisonResult  resultAppVersion = -2;
   ...
   NSNumber* verNumApp;
   NSNumber* verNumAppNew;
   NSNumber* verNumDataExisting;// from DB existing at load
   NSNumber* verNumDataNewDefined = nil;  // used for New when DB !existing at load , i.e. from constants.h
   NSNumber* verNumDataNewXml = nil; //used for NEW from XmlFile in Res/ when DB !existing at load
   ...
   self.versionXML = nil;
   verNumDataNewXml = nil;
   if(!self.doesDbExistAtLoad){ //if !existed at start of run
      ////
      //    GET VERSIONXML DATA from XmlFile >>>> VIRGIN <<<<
      ////
      ...
      // self.versionXMLString has been set by the Parser // "1.50'
      [self setMaxRecipeId:[NSNumber numberWithInt:-1]];//i.e., move all Rxs into DB
      verNumDataNewXml = [numFormater numberFromString:(NSString*)[self versionXMLString]]; //versionXMLString SET BY PARSER
...
[self updateGlobalCategoryArray]; /// NEEDED FOR PARSER                                            //// CategoryArray  ****
      [xmlListReader2	parseXMLFileAtURL:xmlURL parseError:&parseError];
      if (parseError) {
      ...
      // else DB   *** DOES ***  EXIST ***  AT LOAD --       NOT  VIRGIN  !!!!  /// MIGHT BE VERSION UPGRADE
      // COULD BE NORMAL RUN, APPVERSION UPGRADE (W/NEWDAT) OR APPVERSION (W/O NEWDATA) =============   ========
      // An Old DB is present in AppSupport
      //
      // 1. Check XML Data Version in OLD DB
      /// Get Current Version Info from the Sqlite DB       /// MIGHT BE VERSION UPGRADE
      ////
      // IF DB WHICH EXISTS IS FOUND THEN MUST FETCH VersionXml MO. exists and has 1.50
      //
      // THIS IS NOT -- VIRGIN - IF INSTALL RUN THEN NEED TO SAVE VersionXml with new data at ~ 778
...
//self.versionXMLString == nil == self.versionXML     // XXXXXXXX         /// MIGHT BE VERSION UPGRADE
      if(versionArray && [versionArray count]){
         ...
         DLog(@"Fetched:self.versionXML.number=%@",self.versionXML.number);
         ...
         DLog(@"No VersionXML found after fetch."); // X X X  check this for next version 1.2
         //abort();
         //verNumDataExisting = [numFormater numberFromString:(NSString*)DG_DATA_VERSION_FOR_APP_VERSION_ONE ]; //"1.45
      }
      // 2. Get Existing Version of App from Defaults. Use defined Constant if version is not in defaults
      verStringApp = [defaults valueForKey:DG_DEFAULTS_HM_APP_VERSION_KEY];//@"DG_HM_App_Version"
      if (!verStringApp) { // remove this if
         verStringApp = @"1.0";  // APP VERSION
      }
      // CHECK HERE FOR APP VERSIONS > 1.1. 1 INSTALLATION   !!!!       /// MIGHT BE VERSION UPGRADE
      verNumApp = [numFormater numberFromString:verStringApp ];
      verStringAppNew = DG_HM_APP_VERSION_NEW;
      verNumAppNew = [numFormater numberFromString:verStringAppNew ];
      resultAppVersion = [verNumAppNew compare:verNumApp];
      DLog(@"verNumAppNew =%@\nverNumApp=%@",verNumAppNew,verNumApp);
      ...
      //DLog(@"NO VIRGIN:VersionXML data (verNumDataExisting) ready IF needed for writing.");// from old DB
   } //end else DB DOES Exist
   NSError *error5=nil;
   ...
   BOOL bAppVersionUpgrade = (resultAppVersion == NSOrderedDescending) ? YES : NO ; // NEW Version > Existing Version
   BOOL bInstallRun = (!self.doesDbExistAtLoad || bAppVersionUpgrade );
   if (bInstallRun) {
      if (!self.doesDbExistAtLoad ) { //verNumDataExisting has a value if DbExistsAtLoad
         verNumDataExisting = [numFormater numberFromString:@"0.0"];
         // Create New Version MO                                       // VERIFY NO VERSIONXML EXISTS ////   *******
         ...
         ZAssert(self.versionXML == nil, @"self.versionXML != nil");
         self.versionXML = (VersionXML*)newManagedObject;   ///   >>>> VIRGIN <<<<
         if ( self.versionXML == nil) {
            DLog(@"iHMe:No New VersionXML Object Exists");
            exit(225);
         }// self.versionXML == nil
         DLog(@"Virgin Install run. VersionXml MO ready for writing.");
         ////VIRGIN install - VersionXML ready for saving at ~ #638      /// INSTALL RUN
      }
      verNumDataNewDefined = [numFormater numberFromString:(NSString*)DG_RES_DATA_VERSION_NUMBER ]; //"1.50" Constants.h  // use dynamic data V:1.2
      NSNumber *lastestDataVersionNum  = (verNumDataNewXml != nil ) ? verNumDataNewXml : verNumDataNewDefined ;// just to be sure
      resultDataVersion = [lastestDataVersionNum compare:verNumDataExisting];// OK OK
      // fetch newly inserted from MO
      if (resultDataVersion == NSOrderedDescending) { // might be New App Version shipped with Old DatD. /// INSTALL RUN
// NEW DATA PRESENT  -- FETCHED GREATEST RxNum  for EXISTING DB
         [self setMaxRecipeId:[self fetchMaxRecipeIdNum:self.managedObjectContext]];         /// DATAVERSION UPGRADE
         DLog(@"self.maxRecipeId=%@",self.maxRecipeId);
         if(!self.maxRecipeId){
            self.maxRecipeId = [numFormater numberFromString:DG_HM_GREATEST_RX_ID_FOR_APP_VERSION_ONE];
         }
         parseError = nil;    // BOTH APPVERSION AND DATAVERSION UPGRADES
         DLog(@"Check the + 1 below. Actually minimumRecipeId: is not used in call below");
         //WRONG
         self.versionXML.maxRecipeId = self.maxRecipeId;// ADD IF XML HAS GREATER RxId's
         XmlListReader* xmlListReader2 = [[XmlListReader alloc] initForVersionOnly:NO catArray:self.categoryArray minimumRecipeId:self.maxRecipeId.intValue + 1 context:self.managedObjectContext]; // insert if (nRidXml >= minimumRecipeId). If oldDB found, then minRxId == (maxIdInOldDb +1) for this install run
         /// Second Parse, parses all of the file
         /// didParseOK =
...
// VersionXML MO already fetched and stored in self.versionXML   above ~ #679
            ...
            // this is A NON-VIRGIN VERSION UPGRADE
            // IF VERSIONXML COULD BE USED INSTEAD OF USING #DEFINE
            // LET'S USE #DEFINE FOR VERSION 1.1 INSTALL
            //if ( self.versionXML != nil) {     /////// LOAD VersionXML NON-VIrgin         /// INSTALL RUN
            ZAssert(self.versionXML != nil, @"self.versionXML == nil");
            NSNumberFormatter * f = [[NSNumberFormatter alloc] init];               /// DATA VERSION UPGRADE
            [f setNumberStyle:NSNumberFormatterDecimalStyle];
            self.versionXML.number = lastestDataVersionNum;
               //[f numberFromString:self.versionXMLString];// @"1.50"//DG_RES_DATA_VERSION_NUMBER
            ...
            resultAppVersion = [verNumApp compare:[NSNumber numberWithFloat:1.0]];
            if (resultAppVersion == NSOrderedSame) {
      ...
      // SAVE VersionXML - Virgin(#533) NON-Virgin (#652)
      NSError *error6=nil;
      ...
      [defaults setObject:DG_HM_APP_VERSION_NEW forKey:DG_DEFAULTS_HM_APP_VERSION_KEY];
      [defaults setObject:lastestDataVersionNum forKey:DG_DEFAULTS_HM_DATA_VERSION_KEY];
      [defaults synchronize];
   }// end bInstallRun
      //////////////////////
