...
- (IBAction) printAllRecipeAction:(id)sender {
 ...
 [[printView textStorage] appendAttributedString: [recipePrinting ingredients]];
 // Create a couple returns between the ingredients and the directions
 [[printView textStorage] appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n\n"]];
 
 [[printView textStorage] appendAttributedString:[recipePrinting directions]];
 // Create a couple returns between the directions and the comments
 [[printView textStorage] appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n\n"]];
 
 [[printView textStorage] appendAttributedString:[recipePrinting comments]];
...
- (void)tableViewSelectionDidChange:(NSNotification *)aNotification{
   ...
   //[center postNotificationName:@"DG_RemoveSearchFieldStringNotification" object:self userInfo:nil];
   //DLog(@"[[[self categoryArrayController] arrangedObjects]count]=%d",[[[self myCatArrayController] arrangedObjects]count]);
   //DLog(@"[[[self rxFilteredArrayController] arrangedObjects]count]=%d",[[[self rxFilteredArrayController] arrangedObjects]count]);
      //[self.rxFilteredArrayController.searchField setStringValue:@""];
      // clear cell of text by clicking on 'X'
 ...
 **/
   ...
   if (myText) {
      ...
      while (rangeNameStartTag.location != NSNotFound && !isFormatErrorFound && !isSaveError) {
         ...
         rangeIngredients = NSMakeRange(rangeNameEndTag.location + 2 , rangeDirectionsTag.location -1  - rangeNameEndTag.location - 1 );
         //// begin
         ...
         isMissingDirectionsTag = (rangeDirectionsTag.location == NSNotFound || 
                                   (rangeDirectionsTag.location != NSNotFound && rangeDirectionsTag.location > rangeNextNameStartTagAdjusted.location) 
                                   ) ? YES : NO; 
         
         isMissingCommentsTag = (rangeCommentsTag.location == NSNotFound ||
                                 (rangeCommentsTag.location != NSNotFound && rangeCommentsTag.location > rangeNextNameStartTagAdjusted.location)
         ...
         rangeDirections = NSMakeRange(rangeDirectionsTag.location +11 , rangeCommentsTag.location -1  - rangeDirectionsTag.location  - 11);
         ...
         if (rangeNameStartTag.location == NSNotFound) { // if final recipe in file
            rangeComments = NSMakeRange(rangeCommentsTag.location + 9, [myText length] - rangeCommentsTag.location - 9);//
         }else{
            rangeComments = NSMakeRange(rangeCommentsTag.location + 9, rangeNameStartTag.location - rangeCommentsTag.location -9);
         }
